...
// Kontrollo nëse të dhënat janë dërguar me metodën POST
        ...
        if (isset($_POST['edit_player_id']) && isset($_POST['edit_player_name']) && isset($_POST['edit_player_number']) && isset($_POST['edit_team_id'])) {
            $player_id = $_POST['edit_player_id'];
            $player_name = $_POST['edit_player_name'];
            $player_number = $_POST['edit_player_number'];
            $team_id = $_POST['edit_team_id'];
          
            if (!empty($player_id) && !empty($player_name) && !empty($player_number) && !empty($team_id)) {
              $sql = "UPDATE Player SET Name = '$player_name', Number = '$player_number', TeamId = '$team_id' WHERE PlayerId = '$player_id'";
              
              if ($conn->query($sql) === TRUE) {
                echo "Lojtari u përditësua me sukses!";
              } else {
                echo "Error: " . $sql . "<br>" . $conn->error;
              }
            } else {
