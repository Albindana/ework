...
<hr>
  ...
  <ul id="playerList">
    <?php
    // Merr të dhënat e lojtarëve nga baza e të dhënave
    $sql = "SELECT Player.PlayerId, Player.Name AS PlayerName, Player.Number, Team.Name AS TeamName
            FROM Player
            INNER JOIN Team ON Player.TeamId = Team.TeamId";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        echo "<li>" . $row['PlayerName'] . " (Nr: " . $row['Number'] . ") - Ekipi: " . $row['TeamName'] . "
              <form method='GET' action='index.php' style='display:inline;'>
                <input type='hidden' name='edit_player_id' value='" . $row['PlayerId'] . "'>
                <button type='submit'>Edit</button>
              </form>
              </li>";
      }
    } else {
      echo "<li>Nuk ka lojtarë të regjistruar.</li>";
    }
    ?>
  </ul>

  <?php
  // Kontrollo nëse është zgjedhur një lojtar për përditësim
  if (isset($_GET['edit_player_id'])) {
    $edit_player_id = $_GET['edit_player_id'];
    $sql = "SELECT * FROM Player WHERE PlayerId = '$edit_player_id'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
      $row = $result->fetch_assoc();
      $edit_player_name = $row['Name'];
      $edit_player_number = $row['Number'];
      $edit_team_id = $row['TeamId'];
    }
  ?>
    <!-- Forma për përditësimin e lojtarit -->
    <h2>Përditëso Lojtarin</h2>
    <form method="POST" action="back.php">
      <input type="hidden" name="edit_player_id" value="<?php echo $edit_player_id; ?>">
      <input type="text" name="edit_player_name" value="<?php echo $edit_player_name; ?>" required><br><br>
      <input type="number" name="edit_player_number" value="<?php echo $edit_player_number; ?>" required><br><br>
      <select name="edit_team_id" required>
        <option value="">Zgjidh Ekipin</option>
        <?php
        // Merr ekipet dhe shfaq të përzgjedhurin
        $sql = "SELECT * FROM Team";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
          while ($team = $result->fetch_assoc()) {
            $selected = $team['TeamId'] == $edit_team_id ? "selected" : "";
      ...
      <button type="submit">Përditëso Lojtarin</button>
    </form>
  <?php } ?>
</body>
</html>
